...
void main()
{
  ...
  KnownMeteorites::Get(KnownMeteorites::ID::INNISFREE)->Trajectory(points, t, v, h);
  assert(points > 0);
  ...
  std::cout << "----------------------------" << std::endl << std::endl;
  std::cout << "Method's steps: " << (uint32_t)NumericalAlgorithm::THREE_STEP_ADAMS
            << std::endl << std::endl;
  precision.CompareSteps(problem, NumericalAlgorithm::THREE_STEP_ADAMS, std::cout);
  ...
  std::cout << "Method's steps: " << (uint32_t)NumericalAlgorithm::THREE_STEP_ADAMS
            << std::endl << std::endl;
  precision.ComparePerturbations(problem, NumericalAlgorithm::THREE_STEP_ADAMS, (real)1e-3, std::cout);
}

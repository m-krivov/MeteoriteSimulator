...
class PrecisionEstimator
{
  public:
    ...
    void CompareMethods(const VirtualMeteoroid &problem, real dt, std::ostream &str);

    // Estimates the effect of 'dt' on the numerical error
    void CompareSteps(const VirtualMeteoroid &problem, NumericalAlgorithm method, std::ostream &str);

    // Estimates the effect of peturbation for each initial parameter
    void ComparePerturbations(const VirtualMeteoroid &problem, NumericalAlgorithm method, real dt, std::ostream &str);
};

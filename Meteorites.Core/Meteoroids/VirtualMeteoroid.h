...
struct VirtualMeteoroid
{
  public:
    ...
    VirtualMeteoroid() = default;
    VirtualMeteoroid(real H_, real ch_, real rho_, real cd_, real cl_,
                     real m0_, real v0_, real h0_, real gamma0_)
      : H(H_), Ch(ch_), Rho(rho_), Cd(cd_), Cl(cl_),
        M0(m0_), V0(v0_), h0(h0_), Gamma0(gamma0_) { }
    VirtualMeteoroid(const VirtualMeteoroid &) = default;
    VirtualMeteoroid &operator =(const VirtualMeteoroid &) = default;
};
